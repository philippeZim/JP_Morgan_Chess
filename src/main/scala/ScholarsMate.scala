...
object ScholarsMate {
    ...
    def makeMove(board: Vector[Vector[Piece]], move: (Int, Int, Int, Int)): Vector[Vector[Piece]] = {
        val (fromRow, fromCol, toRow, toCol) = move
        val piece = board(fromRow)(fromCol)
        board
          .updated(toRow, board(toRow).updated(toCol, piece))
          .updated(fromRow, board(fromRow).updated(fromCol, Piece(PieceType.EMPTY, Color.EMPTY)))
    }
}