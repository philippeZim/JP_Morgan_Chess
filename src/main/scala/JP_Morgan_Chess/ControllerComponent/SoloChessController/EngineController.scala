package JP_Morgan_Chess.ControllerComponent.SoloChessController

import JP_Morgan_Chess.ChessComponent.ChessTrait
import JP_Morgan_Chess.ChessComponent.RealChess.ChessApiClient
...
class EngineController(using val gameMode : ChessTrait, override var fen : String, var context : ChessContext, var output : String, val depth : Int) extends Observable with ControllerTrait {
    ...
    def play(move : (Int, Int)) : Unit = {
        ...
        val engineMoveString = ChessApiClient.getBestMove(fen, depth)
